...
try {
    ...
    // Hitung angsuran pokok (tetap sama untuk sliding)
    ...
    <table class="header">
        ...
        <tr>
            ...
            <td>:</td>
            <td>Rp. ' . number_format($angsuran_pokok, 0, ',', '.') . '/Bln</td>
        </tr>
        <tr>
            ...
            <td>:</td>
            <td>Rp. ' . number_format($data['sukbung'], 0, ',', '.') . '/Bln</td>
        </tr>
        <tr>
            ...
            <td>:</td>
            <td>Rp. ' . number_format($data['plafond'] + $data['sukbung'] / 100 * $data['plafond'] * $data['jw'], 0, ',', '.') . '/Bln</td>
        </tr>
    </table>
    ...
    for ($i = 1; $i <= $data['jw']; $i++) {
        ...
        // Hitung bunga dengan metode sliding (berdasarkan sisa pokok)
        $angsuran_bunga = ($sisa_pokok * $data['sukbung'] / 100) / 12;
        ...
        <tr>
            ...
            <td class="col-outstanding text-right"></td>
        </tr>';
    }
    ...
    $filename = 'Jadwal Angsuran Sliding - ' . $data['nama'] . ' ' . formatTanggalIndonesia($data['tgl_droping']) . '.pdf';

    // Tulis HTML ke PDF
    $mpdf->SetTitle('Jadwal Angsuran Sliding - ' . $data['nama']);
    $mpdf->WriteHTML($html);
