...
try {
    ...
    // Hitung angsuran pokok, bunga, dan total angsuran
    $angsuran_pokok = $data['plafond'] / $data['jw'];
    $angsuran_bunga = ($data['plafond'] * $data['sukbung'] / 100) / 12;
    ...
    $total_angsuran_formatted = number_format($total_angsuran, 0, ',', '.');
    $plafond_formatted = number_format($data['plafond'], 0, ',', '.');
    
    // Hitung total bunga selama jangka waktu
    $total_bunga = $angsuran_bunga * $data['jw'];
    $total_pembayaran = $data['plafond'] + $total_bunga;
    ...
    <table class="header">
        <tr>
            ...
            <td width="2%">:</td>
            <td width="33%">Rp. ' . $plafond_formatted . '</td>
        </tr>
        <tr>
            <td>No. Rekening</td>
            <td>:</td>
        ...
        <tr>
            ...
            <td>:</td>
            <td>Rp. ' . $angsuran_pokok_formatted . '/Bln</td>
        </tr>
        <tr>
            ...
            <td>:</td>
            <td>Rp. ' . $angsuran_bunga_formatted . '/Bln</td>
        </tr>
        <tr>
            ...
            <td>:</td>
            <td>Rp. ' . $total_angsuran_formatted . '/Bln</td>
        </tr>
    </table>
    ...
    // Sisa pokok dan bunga awal
    $sisa_pokok = $data['plafond'];
    $sisa_bunga = $total_bunga;
    ...
    for ($i = 1; $i <= $data['jw']; $i++) {
        ...
        // Kurangi sisa pokok dan bunga
        $sisa_pokok -= $angsuran_pokok;
        $sisa_bunga -= $angsuran_bunga;
    ...
    $filename = 'Jadwal Angsuran Flat - ' . $data['nama'] . ' ' . formatTanggalIndonesia($data['tgl_droping']) . '.pdf';

    // Tulis HTML ke PDF
    $mpdf->SetTitle('Jadwal Angsuran Flat - ' . $data['nama']);
    $mpdf->WriteHTML($html);
