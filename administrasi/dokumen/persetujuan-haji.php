...
try {
    ...
    <table class="no-border">
        ...
        <tr>
            <td>Suku Bunga</td>
            <td>:</td>
            <td>'.$data['sukbung'].'% ('.ucwords($data['tsukbung']).') / '.$data['cara'].'</td>
        </tr>
        <tr>
            <td>Administrasi</td>
            <td>:</td>
    ...
    <p style="margin-top: 10px;">Angsuran Per Bulan : Rp. '.number_format($data['totang'], 0, ',', '.').',- ('.ucwords($data['ttotang']).')<br>Terhadap Kredit Ini Dikenakan Biaya-biaya sbb :</p>
    ...
    $filename = 'Persetujuan Fasilitas Kredit Haji - ' . $data['nama'] . ' ' . formatTanggalIndonesia($data['tgl_droping']) . '.pdf';

    // Tulis HTML ke PDF
    $mpdf->SetTitle('Persetujuan Fasilitas Kredit Haji - ' . $data['nama'] . ' ' . formatTanggalIndonesia($data['tgl_droping']));
    $mpdf->WriteHTML($html);
