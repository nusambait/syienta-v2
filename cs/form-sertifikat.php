...
// Ubah logika hapus Sertifikat berdasarkan norut
if (isset($_GET['action']) && $_GET['action'] == 'hapus') {
    if (isset($_GET['norut']) && isset($_GET['nik'])) {
        $norut = $_GET['norut'];
        $nik = $_GET['nik'];

        $query = mysqli_query($connect, "DELETE FROM shm WHERE norut='$norut' AND nik='$nik'");
...
if (isset($_GET['action']) && $_GET['action'] == 'lepas') {
    if (isset($_GET['norut']) && isset($_GET['nik'])) {
        ...
        $query = mysqli_query($connect, "UPDATE shm SET status='Tersedia' WHERE norut='$norut' AND nik='$nik'");
...
if (isset($_POST['submit'])) {
    ...
    $query = mysqli_query($connect, "INSERT INTO shm (nik, jenjam, bukkep, suruk, lt, an, almt, kec, kab, blok, tglter, leg, njop, tak, pengikatan, ketjam, status) 
    VALUES ('$nik', '$jenjam', '$bukkep', '$suruk', '$lt', '$an', '$almt', '$kec', '$kab', '$blok', '$tglter', '$leg', '$njop', '$tak', '$pengikatan', '$ketjam', '$status')");
        ...
        <div class="container-fluid">
            ...
            <div class="col-12">
                <div class="d-flex align-items-center justify-content-between mb-4">
                    <h4 class="mb-0 text-gray-800">Form Input Data Sertifikat</h4>
                ...
                <!-- Tambahan: Tabel Data Sertifikat -->
                <div class="card mb-4">
                    <div class="card-header">
                        <h5 class="mb-0">Data Sertifikat Nasabah <?php echo $nasabah['nama']; ?></h5>
                    </div>
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-bordered table-hover">
                                ...
                                <tbody>
                                    <?php
                                    $query_sertifikat = mysqli_query($connect, "SELECT * FROM shm WHERE nik='$nik'");
                                    $no = 1;
                                    if (mysqli_num_rows($query_sertifikat) > 0) {
                                        while ($data_sertifikat = mysqli_fetch_array($query_sertifikat)) {
                                    ?>
                                            <tr>
                                                <td><?php echo $no++; ?></td>
                                                <td><?php echo $data_sertifikat['jenjam']; ?></td>
                                                <td><?php echo $data_sertifikat['bukkep']; ?></td>
                                                <td><?php echo $data_sertifikat['suruk']; ?></td>
                                                <td><?php echo $data_sertifikat['lt']; ?></td>
                                                <td><?php echo $data_sertifikat['an']; ?></td>
                                                <td><?php echo $data_sertifikat['status']; ?></td>
                                                <td class="d-flex justify-content-center">
                                                    <button type="button" class="btn btn-warning btn-sm me-1"
                                                        onclick="editSertifikat('<?php echo $data_sertifikat['norut']; ?>')">
                                                        <i class="bi bi-pencil"></i>
                                                    </button>
                                                    <?php if ($data_sertifikat['status'] != 'Tersedia'): ?>
                                                        <button type="button" class="btn btn-success btn-sm me-1"
                                                            onclick="lepasSertifikat('<?php echo $data_sertifikat['norut']; ?>')">
                                                            <i class="bi bi-unlock"></i>
                                                        </button>
                                                    <?php endif; ?>
                                                    <button type="button" class="btn btn-danger btn-sm"
                                                        onclick="hapusSertifikat('<?php echo $data_sertifikat['norut']; ?>')">
                ...
                <div class="card">
                    <div class="card-body">
                        <h5 class="card-title mb-4 bg-judul-card">Form Tambah Data Sertifikat</h5>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select class="form-control" name="jenjam" required>
                                        ...
                                        <option value="Sebidang Tanah Darat dan Bangunan">Sebidang Tanah Darat dan
                                            Bangunan</option>
                                        ...
                                        <option value="Sebidang Bangunan">Sebidang Bangunan</option>
                                    </select>
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label class="form-label required">Bukti Kepemilikan</label>
                                    <input type="text" class="form-control" name="bukkep"
                                        placeholder="Nama Dokumen / No Jaminan ex (SHM No. 12345)" required>
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label class="form-label required">Surat Ukur</label>
                                    <input type="text" class="form-control" name="suruk" required>
                                </div>
                                <div class="col-md-6 mb-3">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="number" class="form-control" name="leg" required>
                                </div>
                                <div class="col-md-6 mb-3">
    ...
    <script>
        function hapusSertifikat(norut) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = 'form-sertifikat.php?action=hapus&norut=' + norut +
                        '&nik=<?php echo $nik; ?>';
                }
            })
        }
        ...
        function lepasSertifikat(norut) {
            Swal.fire({
                title: 'Apakah Anda yakin?',
                text: "Status Sertifikat akan diubah menjadi Tersedia!",
                icon: 'warning',
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    window.location.href = 'form-sertifikat.php?action=lepas&norut=' + norut +
                        '&nik=<?php echo $nik; ?>';
                }
            })
        }

        function editSertifikat(norut) {
            window.location.href = 'edit-sertifikat.php?norut=' + norut + '&nik=<?php echo $nik; ?>';
        }
    </script>
</body>
